...
export default {
  ...
  computed: {
    previewSourceCode() {
      return this.sourceCode.replace(/'\.\.\/\.\.\/index'/g, `'@cro/toy-ui'`);
    },
  },
  async mounted() {
    ...
    if (this.compName && this.demoName) {
      if (isDev) {
        ...
        this.sourceCode = await fetch(`${isDev ? '' : '/toy-ui'}/packages/${this.compName}/docs/${this.demoName}.vue`).then((res) => res.text());
      }
    }
    await this.$nextTick();
    Prism.highlightAll();
